...
mod ble;
use ble::{BleDevice, HeartRateData, scan_heart_rate_devices, connect_and_listen_heart_rate};
...
fn set_window_to_desktop_bottom(window: &tauri::WebviewWindow) {
    ...
    unsafe {
        ...
        // 设置为工具窗口（不显示在任务栏）
        use windows::Win32::UI::WindowsAndMessaging::{GetWindowLongW, SetWindowLongW, GWL_EXSTYLE, WS_EX_TOOLWINDOW, WS_EX_NOACTIVATE};
        let ex_style = GetWindowLongW(hwnd, GWL_EXSTYLE);
        SetWindowLongW(hwnd, GWL_EXSTYLE, ex_style | WS_EX_TOOLWINDOW.0 as i32 | WS_EX_NOACTIVATE.0 as i32);
        
        // 置于底层
        let _ = SetWindowPos(hwnd, HWND_BOTTOM, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE);
    }
}
...
async fn save_window_position(x: i32, y: i32) -> Result<(), String> {
    ...
    std::fs::write(&position_file, data.to_string())
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if let Some(m) = monitor {
                ...
                let y = 50.0;
                let _: Result<(), _> = window.set_position(
            ...
            // 创建托盘菜单
            let show_item = tauri::menu::MenuItem::with_id(app, "show", "显示/隐藏", true, None::<&str>)?;
